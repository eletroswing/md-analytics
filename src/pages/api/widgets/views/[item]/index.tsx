...
export default async function handler(
  ...
  const total_unique = await connection?.query(
    ...
    SELECT COUNT(DISTINCT ip) AS unique_views_last_hour
    FROM tracking
    WHERE pixel_id = $1
      AND created_at >= NOW() - INTERVAL '1 hour'
),
current_hour_views AS (
    SELECT COUNT(DISTINCT ip) AS unique_views_current_hour
    FROM tracking
    WHERE pixel_id = $1
      AND created_at >= NOW() - INTERVAL '1 hour' AND created_at <= NOW()
),
unique_v AS (
    SELECT COUNT(DISTINCT ip) AS unique_views
    FROM tracking
    WHERE pixel_id = $1
)
...
CROSS JOIN unique_v;
  ...
  const cachedSvg = await redis?.get(`view-widget:${req.query.item}`);
  let renderBuffer: any;

  if(cachedSvg) {
    const svg = await transformToSvg(renderer(total, growth), 300);
    await redis?.set(`view-widget:${req.query.item}`, svg)    
  ...
  return (
    <div className="px-4 pt-6 shadow-lg bg-[#1f1f23] shadow-blue-100 flex flex-col w-[300px] rounded-md">
      <div className="flex mb-2">
        ...
        <p className="mt-4 font-medium ml-4 text-2xl text-white">Unique Views</p>
      </div>
      <p
